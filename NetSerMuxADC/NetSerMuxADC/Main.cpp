...
//variables for which device is being tested and what number file the loop is on
int deviceid;
int filenum;
...
int main(int argc, char* argv[]) {
	//extracting parameters from the argument array
	string deviceidstring(argv[1]);
	string filenumber(argv[2]);
	string comnum(argv[3]);
	deviceid = stoi(deviceidstring);
	filenum = stoi(filenumber);
	//open output file for the ADC data
	...
	setserial(&serialh, &serialparams,comnum);
	CloseHandle(serialh);
	setserial(&serialh, &serialparams,comnum);
	...
	while (1) {
		...
		//if start is received first then a new data file is coming, if 'done' is received then shut server down
		if (init == "start") {
			//send a confirmation message (arbitrary message
			send(new_socket, message, strlen(message), 0);
			//receive the name of the text file that is sent
			receivesize = recv(new_socket, incoming, 1000, 0);
			incoming[receivesize] = '\0';
			...
			//function that sends EGM data to either the ICD or the algorithm, also gets ICD shock data from the microcontroller and makes and sends log files for ICD device 
			sendEGMfile(&serialh, pointAStop, pointASbot, pointVStop, pointVSbot, pointSStop, pointSSbot,
				...
				buffer_data_in_Z, textfilename, &new_socket, &struct4_send, deviceid,filenum);

			cout << "Finished " << textfilename << endl << endl;
			//only wait on ICD algorithm if it is the device beign tested
			if (deviceid == 2) {
				cout << "Sending First Message" << endl;
				// wait for the ICD algorithm to end, send message to tell other program that it can move on
				send(new_socket, message, strlen(message), 0);
				cout << "rec NI done" << endl;
				receivesize = recv(new_socket, incoming, 1000, 0);
				cout << "Send SEcond message" << endl;
			}
			send(new_socket, message, strlen(message), 0);
