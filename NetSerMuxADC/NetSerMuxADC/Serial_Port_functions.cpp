...
//set serial function
void setserial(HANDLE *serial_handle, DCB *serialparams, std::string comport) {
	std::string temp = "\\\\.\\COM" + comport;
	
	*serial_handle = CreateFile(temp.c_str(), GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
...
void senddata(HANDLE *serial_handle, char *AS_top, char *AS_bot, char *VS_top, char *VS_bot, char *SS_top, char  *SS_bot
	...
	int error;
	//write data byte by byte, for loops are used to prevent data over run in the microcontrolelr
...
void startmbed(HANDLE *serial_handle, int loop_num) {
	...
	for (int l = 0; l < 10000; l++);
	if (loop_num == 1)error = WriteFile(*serial_handle, &muxICD, 1, NULL, NULL);
...
void Receive_ADC_Data(HANDLE *serial_handle,float *ADC_voltage_data) {
	...
	//fill buffer with voltage values, 30000 samples, 1 sample for each EGM sample
...
void make_log(int send_num,double *buf_data_X, double *buf_data_Y, double *buf_data_Z, float *buf_ADC_voltage, std::string textfilename) {
	//open file for the log file
...
//function for sending the EGM file to either the algorithm or the ICD device
void sendEGMfile(HANDLE *serial_handle, char *pointAStop, char *pointASbot, char* pointVStop, char *pointVSbot, char *pointSStop, char *pointSSbot,
	...
	double *buffer_data_in_Z, std::string textfilename, SOCKET *socket, struct Filestruct *struct_4log_send, int deviceid, int filenum) {
	...
	char rec;
		...
		startmbed(serial_handle, deviceid);
		...
		if (deviceid==1) {
			...
			make_log(0, buffer_data_in_X, buffer_data_in_Y, buffer_data_in_Z, ADCvoltage, textfilename);
